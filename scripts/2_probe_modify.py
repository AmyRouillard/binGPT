        ...
        for w in ["trained", "random"]:
            ...
            for loader_name, loader in zip(
                ...
                for i, batch in enumerate(loader):
                    ...
                    print(
                        f"Changed: {mask.sum().item()-N_unchanged0}/{mask.sum().item()} ({(mask.sum().item()-N_unchanged0)/mask.sum().item():.4f})"
                    )
                    print(
                        f"Changed: {(~mask).sum().item()-N_unchanged1}/{(~mask).sum().item()} ({((~mask).sum().item()-N_unchanged1)/(~mask).sum().item():.4f})"
                    )
