...
class ConwayGame:
    ...
    def randomize_grid(self):
        probability_of_one = np.clip(
            np.random.normal(self.order_mean, self.order_std), 0, 1
        )
        for i in range(0, self.GRID_WIDTH):
            for j in range(0, self.GRID_HEIGHT):
                self.grid[i, j] = random.random() < probability_of_one

    def randomize_grid_uniform(self, probability_of_one=0.5):
        for i in range(0, self.GRID_WIDTH):
            for j in range(0, self.GRID_HEIGHT):
                self.grid[i, j] = random.random() < probability_of_one
    ...
    def get_state_as_string(self, grid=None):
        if grid is None:
            grid = self.grid
    ...
    def set_grid_from_string(self, grid_string):
        if len(grid_string) != self.GRID_WIDTH * self.GRID_HEIGHT:
            raise ValueError(
                "The length of the input string does not match the grid dimensions."
            )
        grid_array = np.array([int(char) for char in grid_string]).reshape(
            (self.GRID_WIDTH, self.GRID_HEIGHT)
        )
        self.grid = grid_array.astype(bool)

    def generate_sets(self, A=100, N=32, I=2, s=0.5, e=0.5):
